...
def create_dataset(config, input_dir, output_dir, verbose=True):
    ...
    '''
    dataset = LibriTTSMetaData(input_dir)
    ...
    print('-----'*10 + 'begin to preprocess libritts dataset' + '-----'*10)
    for example in tqdm.tqdm(dataset):
        index, file_path, speaker_id, chapter_id, text = example
        ...
        file_names.append([base_name, speaker_id, chapter_id, text, mel.shape[-1], audio.shape[-1]])
